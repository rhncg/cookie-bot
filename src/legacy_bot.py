...
#                 content=f"You gambled {numerize(self.amount, 2)} cookies and won {numerize(gamble_result * boost, 2)} cookies! Your new balance is {numerize(balance, 2)}.",
...
# @bot.command()
# async def gamble(ctx, quick_selection: discord.Option(str, choices=['all', 'half']) = None, amount: int = None): # type: ignore
#     data = await get_data(ctx.author.id)
